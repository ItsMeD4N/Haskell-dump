...
maxRedSum t
  ...
  | otherwise =
      let l = left t
          ...
          ambilAkar =
            akar t
              + (if isTreeEmpty l then 0 else maxRedSum (left l) + maxRedSum (right l))
              + (if isTreeEmpty r then 0 else maxRedSum (left r) + maxRedSum (right r))
          tidakAmbilAkar = maxRedSum l + maxRedSum r
       in max ambilAkar tidakAmbilAkar
