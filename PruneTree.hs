...
pruneTree p d =
  if isTreeEmpty p then
    ...
    Node (akar p) Empty Empty
  else if isOneElmt p then
    p
  else if isUnerLeft p then
    Node (akar p) (pruneTree (left p) (d - 1)) Empty
  else if isUnerRight p then
    Node (akar p) Empty (pruneTree (right p) (d - 1))
  else if isBiner p then
    Node (akar p) (pruneTree (left p) (d - 1)) (pruneTree (right p) (d - 1))
  else
    p
