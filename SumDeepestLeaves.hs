...
sumDeepestLeaves p =
  if isTreeEmpty p then
    ...
    sumAtDepth p (maxDepth p)

maxDepth :: (BinTree Int) -> Int
maxDepth p =
  if isTreeEmpty p then
    0
  else
    1 + max (maxDepth (left p)) (maxDepth (right p))

sumAtDepth :: (BinTree Int) -> Int -> Int
sumAtDepth p d =
  if isTreeEmpty p then
    0
  else if d == 1 && isOneElmt p then
    akar p
  else
    sumAtDepth (left p) (d - 1) + sumAtDepth (right p) (d - 1)
