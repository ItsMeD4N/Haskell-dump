...
withdraw targetName amount = map (\p -> if name p == targetName then p { balance = balance p - amount } else p)
...
deposit targetName amount = map (\p -> if name p == targetName then p { balance = balance p + amount } else p)
